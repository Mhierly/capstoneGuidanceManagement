      ...
      $(document).ready(function() {
          var table = $('#studentList').DataTable({
            ...
            createdRow: function(row, data, dataIndex) {
                if (data.profile_status.includes('Completed')) {
                    $(row).addClass('bg-primary text-white');
                } else if (data.profile_status.includes('Need to update')) {
                    $(row).addClass('bg-danger text-white');
                }
            },
            columnDefs: [
                { targets: 2, orderable: false, searchable: false } // Adjust the column index as needed
            ]
        });
